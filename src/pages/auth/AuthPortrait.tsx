...
interface AuthPortraitProps {
  /** La portada quedó atrás: al tocar "Continuar" con sesión, o al autenticarse sin ella. */
  onIntroSeen: () => void;
}

/**
 * La portada en móvil: la promesa, el cachorro y una sola acción.
 *
 ...
 */
function AuthPortrait({ onIntroSeen }: AuthPortraitProps) {
  const { user } = useAuth();
  ...
  return (
    <div className="relative w-full h-svh overflow-hidden bg-brand-gradient">
      ...
      <div
        className={`absolute inset-x-0 top-0 z-20 px-5 pt-[calc(5rem+env(safe-area-inset-top))] transition-opacity duration-300 ${
      ...
      >
        <button
          ...
          className="w-full h-14 rounded-full bg-white text-brand-dark text-base font-extrabold shadow-float active:scale-95 transition-transform"
        >
